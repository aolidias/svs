...
(function () {
	 ...
	 */
		...
		.filter('fromNow', ['$window', '$log', 'angularMomentConfig', function ($window, $log, angularMomentConfig) {
			return function (value) {
				...
				return applyTimezone(moment, angularMomentConfig.timezone, $log).fromNow();
			};
		}])
		...
		.filter('amDurationHours', ['$window', function ($window) {
			...
			return function (value) {
				...
				// else assume the given value is already a duration in a format (miliseconds, etc)
				return $window.moment.duration(value).asHours().toFixed(1);
			};
		}]);
